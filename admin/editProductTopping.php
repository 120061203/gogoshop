...
<body>
    <?
        ...
        $sql_query = "select * from toppings where topping_id='".$topping_id."'";
        $result=mysql_query($sql_query);
        $row=mysql_fetch_array($result);
    ?>
    <nav>
                ...
                <form action="editProductTopping_result.php" method="post">
                    <table>
                        <tbody>
                            <tr>
                                <td>配料名稱</td>
                                <td><input type="text" maxLength="10" size="30" name="toppingName" placeholder="請輸入配料名稱" value=<?echo $row[1]?> required></td>
                            </tr>
                            <tr>
                                <td>配料價格</td>
                                <td><input type="number" maxLength="20" size="30" name="toppingPrice" placeholder="請輸入配料價格" value=<?echo $row[2]?> required></td>
                                <input type="hidden" name="topping_id" value=<?echo $topping_id?>><!--傳送topping_id-->
                            </tr>
                        </tbody>
                    </table>
